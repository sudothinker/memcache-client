...
class MemCache
  ...
  def set(key, value, expiry = 0, raw = false)
    ...
    with_server(key) do |server, cache_key|
      ...
      ActiveRecord::Base.logger.info "#{Time.now.strftime("%Y-%m-%d %H:%M:%S ")} Setting key/value #{key}\n#{value}" if value.to_s.size > 800000
  ...
  def add(key, value, expiry = 0, raw = false)
    ...
    ActiveRecord::Base.logger.info "#{Time.now.strftime("%Y-%m-%d %H:%M:%S ")} Adding key/value #{key}\n#{value}" if value.to_s.size > 800000
